...
iess_pib_pred <- ggplot() + geom_line( data = aux_his, aes( x = anio, y = pib_nominal, group = 1 ),
...
iess_ipc_hist  <- ggplot( data = aux, aes( x = Anio, y= valor, group = tipo ) ) +
  geom_line( aes( linetype = tipo, color = tipo ) ) +
  # scale_x_continuous( breaks = waiver(), limits = x_lim, labels = waiver() ) +
  scale_y_continuous( breaks = y_brk, labels = y_lbl, limits = y_lim,
                      sec.axis = sec_axis( ~.*(91/106),
                                           name = "Inflación",
                                           labels = function( b ) { paste0( round( b * 1, 0 ), "%" )} ) ) +
